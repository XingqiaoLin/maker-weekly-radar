...
12. Merge the complete strict-pass set, then apply the final source mix as soft minimum targets: YouTube 5+, Reddit 4+, crowdfunding (Kickstarter + Indiegogo) 1+, Hackaday 1+, and four initial round-robin slots for the other platforms. Preserve total valid project count up to 15 and refill missing or remaining slots by strict editorial score. Enforce one explicit diversity ceiling: Instructables may contribute at most 3 final selections. All other platforms have no hard maximum. Validate, sort, and render:
...
- For YouTube and Reddit, discover broadly from the default RSS bundles (at least 100 verified channels and 20 communities). Preserve Reddit RSS text, author, media, the exact combined-feed position, and the feed URL. Prefer approved Installed-App OAuth or audited browser evidence for Reddit heat; otherwise admit only official combined weekly RSS ranks 1–50 as proxy heat, never as a passing score. Keep the legacy Unicode-safe public-page/JSON enrichment available only for custom diagnostic profiles. Do not narrow raw discovery with title-only Maker patterns.
