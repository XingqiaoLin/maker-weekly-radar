...
- For YouTube and Reddit, discover broadly from the default RSS bundles (at least 29 verified channels and 20 communities). Preserve Reddit RSS text, author, media, the exact combined-feed position, and the feed URL. Prefer approved Installed-App OAuth or audited browser evidence for Reddit heat; otherwise admit only official combined weekly RSS ranks 1–10 as proxy heat, never as a passing score. Keep the legacy Unicode-safe public-page/JSON enrichment available only for custom diagnostic profiles. Do not narrow raw discovery with title-only Maker patterns.
