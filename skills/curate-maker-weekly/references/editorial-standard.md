...
- 记录周开始、周结束、上周快照日期和当前执行日期。
- 候选仅可通过一种时间条件：
  - `first_release`：项目、帖子、视频或众筹页在目标周首次上线。
  - `breakout`：项目此前存在，上周快照未达门槛，本周快照首次达门槛。
- 本周首发不得同时标记翻红。
- 首次发现但未达标的项目进入本周快照；只有下一周新达标才可翻红。
- 没有真实上周快照时，只能建立基线，不能追溯猜测翻红。
...
数据不可见、不可核验、币种无法可靠换算或来源冲突时，热度门失败。RSS 顺序不是热度数据。
...
严格使用平台表。每个数字记录抓取时间和原始页面。不得用估算、第三方声量或跨平台热度替代所属平台门槛。
...
`category` 只能是：`社会价值`、`极客硬核`、`艺术科技交互`、`生活方式社群`。`entry_type` 只能是 `first_release` 或 `breakout`。仅当创作者明确寻求资金、合作、设备或资源时，增加 `invitation_signal`，并附原始证据 URL。

## 8. 最终输出结构

开头列出：时间范围、检索平台数、初始候选数、通过数、首发数、翻红数。

每项依次输出：原始链接、辅助证据、类别、入选类型、首次发现日期、本周热度、上周热度（仅翻红）、创作者、项目简介、制作与技术路径、项目门至少三项证据、卓越方向、卓越对标话术、三条红线检查、六项评分、总分、两到三句入选理由，以及有证据时才出现的邀约信号。
