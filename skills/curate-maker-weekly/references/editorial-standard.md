...
对于 Hackaday、Make、The Verge、Tom’s Hardware 等媒体页，标题或摘要本身必须明确指向具体实体造物及制作行为，正文再提供成品和过程证据。导航、侧栏、相关文章、食谱或购买指南中的词不得用于通过零号门。媒体转述可进入候选预筛，但最终必要条件仍须追溯实际制作者及原始证据。
...
固定流水线为：平台抓取 → raw 审计 → Make Something Gate → 时间门 → 热度门 → 完整跨平台研究池 → 五关 → 三条红线 → 卓越度 → 六维评分 → 最终来源混排 → 最终 Top 15。基础三门后不得提前裁成 15 条，必须保留全部通过者接受严格评审。只有全部严格标准通过并完成评分后，最终选择才应用 YouTube 至少 5、Reddit 至少 4、Kickstarter 与 Indiegogo 合并众筹池至少 1、Hackaday 至少 1，另为其余平台初始轮转 4 个名额；任一池无合格项目或仍有剩余名额时按严格编辑评分自动回填。以上来源目标是软最低目标；唯一硬性来源上限是 Instructables 最多 3 条。被上限排除后应继续从其他严格通过者中回填，尽量保持总数接近 15，不降门槛、不凑数。
...
`decisions.json` 必须完整覆盖研究池：`items` 放通过五关、三条红线、卓越度并已评分的项目；`rejections` 放未通过者及其首个失败阶段。缺少任一研究候选的结论时，最终选择必须报错：
...
```json
{
  ...
  "rejections": [
    {
      "id": "candidate-id-that-failed",
      "failed_stage": "project_gate",
      "rejection_reason": "只有两个项目门维度具备直接证据",
      "evidence_url": "https://primary.example/project"
    }
  ]
}
...
`failed_stage` 只能是 `category_gate`、`project_gate`、`necessary_conditions`、`excellence`、`red_line` 或 `evidence_verification`。最终 5+/4+/1+/1+ 混排只读取 `items` 中已经通过全部严格标准并完成六维评分的项目。
