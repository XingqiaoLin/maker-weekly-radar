...
固定流水线为：平台抓取 → raw 审计 → Make Something Gate → 时间门 → 热度门 → 完整跨平台研究池 → 五关 → 三条红线 → 卓越度 → 六维评分 → 最终来源混排 → 最终 Top 15。基础三门后不得提前裁成 15 条，必须保留全部通过者接受严格评审。只有全部严格标准通过并完成评分后，最终选择才应用 YouTube 至少 5、Reddit 至少 4、Kickstarter 与 Indiegogo 合并众筹池至少 1、Hackaday 至少 1，另为其余平台初始轮转 4 个名额；任一池无合格项目或仍有剩余名额时按严格编辑评分自动回填。所有来源组都可以超过最低目标，不降门槛、不凑数。
