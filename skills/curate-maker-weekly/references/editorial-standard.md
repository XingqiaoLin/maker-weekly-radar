...
固定流水线为：平台抓取 → raw 审计 → Make Something Gate → 时间门 → 热度门 → 跨平台去重与统一候选评分 → 来源软配额混排候选 Top 15 → 五关三红线 → 最终 Top 15。候选初始目标为 YouTube 至少 5、Reddit 至少 4、Kickstarter 与 Indiegogo 合并众筹池 1、Hackaday 1、其余平台轮转 4；任一池不足或仍有剩余名额时按全局分数自动回填，YouTube 与 Reddit 可以超过 5 和 4。不降门槛、不凑数，最终编辑评分仍以质量为准。
