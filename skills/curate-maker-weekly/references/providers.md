...
Every enabled `sources[]` entry is an independent platform source. Its bounded fetch results first enter `raw-discoveries.json`; they do not count as Maker candidates. Run the Make Something Gate, time gate, and heat gate, then retain every pass in the complete research pool. Do not apply source targets or a Top-15 cap before the five gates, three red lines, excellence check, and six-dimension scoring. Final selection then uses minimum targets of YouTube 5+, Reddit 4+, crowdfunding 1+, and Hackaday 1+, plus four initial round-robin slots for all other platforms. Instructables has a hard final maximum of 3; skipped Instructables and all empty or remaining slots refill from other strict passes by editorial score. These rules are never eligibility exemptions. All publication lookbacks are calculated in UTC from `--as-of` or the current time. `--as-of` is the publication cutoff, not a heat-observation cutoff; public heat is collected at execution time and keeps its real `metrics_captured_at`.
...
Persist successful official-widget observations in `evidence_cache_path`. When a later widget request is temporarily blocked, reuse a cached observation only if its official launch date belongs to the same target issue; retain its original capture time and `kickstarter_official_widget_cache` provenance. Never reuse cached metrics for a different week.
...
Use RSS or Atom without secrets. Set `feed_url` for one feed or `feed_urls` to merge several feeds into one platform-level source before global candidate ranking. `static_metrics` may be used only when the feed URL itself proves the property—for example Hackster's `by=featured` feed may set `featured: true`. For date-sensitive editorial sources, prefer official daily/monthly archives, paged Featured feeds, or official sitemaps so a later execution does not lose target-week entries that rolled out of the current feed.
...
For no-account YouTube, set `detail_enrichment: youtube_public`. The formal default bundles at least 100 verified Maker/engineering channel feeds, normalizes video and Shorts links, and records visible video views and channel subscribers. For Reddit, use `detail_enrichment: reddit_fallback` with one official combined `top/.rss?t=week` feed covering at least 20 communities. It preserves RSS author, text, image/video URLs, process language, exact feed position, and feed URL, then tries Installed-App OAuth and audited browser evidence for exact heat. When exact interactions remain unavailable and `weekly_rss_rank_fallback.enabled=true`, ranks 1–50 may pass only as `reddit_weekly_rss_rank` proxy evidence with `exact_score_available=false`. Exact score/comments always take precedence; a verified total below 500 fails even at RSS rank 1. Other RSS feeds and hand-entered ranks are ineligible. The legacy `reddit_old` mode retains bounded Unicode-safe old-page/JSON diagnostics for custom profiles but is not formal heat evidence. Respect HTTP 429 and `Retry-After`; never bypass limits. The formal default deliberately does not use title-only Maker filters.

Use the expanded default: YouTube reaches 25,000 views or 10,000 channel subscribers; Reddit score plus comments reaches 500, or uses official combined weekly RSS rank 1–50 only when exact interactions are unavailable; Kickstarter reaches US$5,000 or 40 backers. These gates expand editorial research only; keep all physical, time, authorship, process, and final editorial gates unchanged.
