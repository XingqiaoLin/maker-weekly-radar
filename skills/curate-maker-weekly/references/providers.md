...
For no-account YouTube, set `detail_enrichment: youtube_public`. The formal default bundles at least 29 verified Maker/engineering channel feeds, normalizes video and Shorts links, and records visible video views and channel subscribers. For no-account Reddit, set `detail_enrichment: reddit_old`; use a small number of multi-subreddit RSS bundles to cover at least 20 communities without triggering one anonymous request per community. Preserve the RSS-embedded author, post text, image/video URLs, and process language before verifying score and comments on the public old-Reddit page. Respect HTTP 429 and `Retry-After`; use bounded retries and a small configured pause rather than bypassing limits. The formal default deliberately does not use title-only Maker filters. YouTube must reach 200,000 views or 50,000 channel subscribers; Reddit must reach score plus comments of 5,000.
