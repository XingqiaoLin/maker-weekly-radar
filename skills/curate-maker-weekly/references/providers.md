...
Every enabled `sources[]` entry is an independent platform source. Its bounded fetch results first enter `raw-discoveries.json`; they do not count as Maker candidates. Run the Make Something Gate, time gate, and heat gate before cross-platform deduplication and global candidate scoring. The default candidate mix starts with minimum targets of YouTube 5+, Reddit 4+, crowdfunding 1+, and Hackaday 1+, plus four initial round-robin slots for all other platforms. Empty and remaining slots refill from global score order, so every source group—including crowdfunding and Hackaday—may exceed its initial minimum. These are diversity targets after all hard gates, never eligibility exemptions. All publication lookbacks are calculated in UTC from `--as-of` or the current time. `--as-of` is the publication cutoff, not a heat-observation cutoff; public heat is collected at execution time and keeps its real `metrics_captured_at`.
